...
**Use the training and test datasets from the *titanic* R package to try the following machine learning classification algorithms.**  
...
**Once you are comfortable running all of the models see if you can figure out how to use cross-validation to pick the best model.**
...
To begin, I processed the data according to the steps provided in the assignment and ran the models, starting with the null, as given in the sample code provided. I then replaced some of the omitted variables and re-analyzed the results, finding greater accuracy with the expanded dataset.  

After generating predictions based on all of the requested models using the provided code, I proceeded with an attempt to optimize the accuracy of the predictions by tuning the algorithms with cross-validation. Each model or algorithm was cross-validated with specific sets of parameters, I compared Cohen's \(\kappa\) for each set of predictions and the most accurate classifiers are reported for each algorithm. Those results are tabulated here:      
...
I produced two classifiers that tie for most accurate. The best classifiers I produced used random forests with the `ranger` package and a boosted C5.0 model with `xgboost`. 
...
I used this classifier to generate the predictions that I turned in along with this document.  
...
Having looked closely at the null model we may proceed to build our classifiers. We will begin with kNN.  
...
Accuracy and kappa ofour random forest.  
...
Our last attempt at building a classifier will use a Naive Bayes model from the `klaR` package.   
